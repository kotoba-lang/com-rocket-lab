(ns rocket-lab.main
  "Kotodama WASM entrypoint for the rocket_lab clean-room actor (L4) — Clojure port.
  ...
  facts (`rocket_lab.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rocket_lab")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rocket_lab.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rocket_lab-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rocket_lab-compat/ready)
